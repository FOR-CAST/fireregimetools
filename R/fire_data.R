...
## Shared body: read poly shapefile(s), drop invalid geometries, harmonise YEAR +
...
#' NBAC vintages. Invalid geometries are dropped, records are filtered to
#' `fire_years` and `SIZE_HA >= 1` ha, then projected + cropped to `study_area`.
#'
...
load_nbac_polys <- function(nbac_shp, study_area, fire_years) {
  .load_fire_polys(
    nbac_shp, study_area, fire_years,
...
#' `POLY_HA`, or `HECTARES`. Invalid geometries are dropped, records are filtered
#' to `fire_years` and `SIZE_HA >= 1` ha, then projected + cropped to `study_area`.
...
load_nfdb_polys <- function(nfdb_shp, study_area, fire_years) {
  .load_fire_polys(
    nfdb_shp, study_area, fire_years,
    year_cols = c("YEAR", "FIRE_YEAR"),
